...
if ! command -v jq &>/dev/null || ! command -v curl &>/dev/null; then
  log_warn "Installing jq and curl on live USB..."
...
read -p "Select ROOT partition index (Will be formatted BTRFS): " ROOT_IDX
ROOT_PART="${PART_PATHS[$((ROOT_IDX - 1))]}"
read -p "Select EFI partition index (Will be formatted FAT32): " EFI_IDX
